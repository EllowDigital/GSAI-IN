  ...
  return (
    <section className="relative min-h-[68vh] md:min-h-[78vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative w-full flex flex-col items-center justify-center px-2 xs:px-4 py-14 xs:py-16 md:py-28 z-10">
        ...
        <motion.div
          className="absolute left-1/2 transform -translate-x-1/2 bottom-4 xs:bottom-6 flex flex-col items-center"
        ...
        {/* Slider controls/dots (improved for mobile/touch) */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
          {bgImages.map((_, idx) => (
            <button
              key={idx}
              className={`w-3 h-3 xs:w-3.5 xs:h-3.5 rounded-full border-2 border-white transition focus:outline-yellow-400 ${imgIndex === idx ? "bg-yellow-400 shadow-lg" : "bg-white/30"}`}
