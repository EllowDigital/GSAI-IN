...
export default function AttendanceManager() {
  ...
  const markAttendanceMutation = useMutation({
    mutationFn: async ({
      ...
      const existing = (dayAttendance as any[]).find(
        (a: any) => a.student_id === studentId
      );
      if (existing) {
        // Already marked — do NOT allow changes
        throw new Error(
          'Attendance already marked for this student today. It cannot be changed.'
        );
      }
      // Insert new record
  ...
  const markCheckout = useMutation({
    mutationFn: async (studentId: string) => {
      ...
      if (!existing) {
        toast.error('Student not checked in');
        return;
      }
  ...
  const markAllPresent = async () => {
    const unmarked = students.filter(
      (s) => !(dayAttendance as any[]).find((a: any) => a.student_id === s.id)
    );
  ...
  const attendanceMap = useMemo(() => {
    ...
    (dayAttendance as any[]).forEach((a: any) => {
      map[a.student_id] = a;
    });
    return map;
  }, [dayAttendance]);
    ...
    (monthAttendance as any[]).forEach((a: any) => {
      if (stats[a.student_id]) {
        ...
        if (a.status === 'present' || a.status === 'half_day')
          stats[a.student_id].present++;
        if (a.status === 'absent') stats[a.student_id].absent++;
        if (a.status === 'late') {
          stats[a.student_id].late++;
          stats[a.student_id].present++;
        }
      }
    });
  ...
  const todayStats = (() => {
    ...
    const absent = (dayAttendance as any[]).filter(
                ...
                <TableBody>
                  {filteredStudents.map((student) => {
                    ...
                    return (
                      <TableRow
                        key={student.id}
                        className={isLocked ? 'bg-muted/20' : ''}
                      >
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            {isLocked ? (
                              ...
                              <>
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="h-7 px-1.5 text-yellow-600"
                                  title="Late"
                                  onClick={() =>
                                    markAttendanceMutation.mutate({
                                      studentId: student.id,
                            ...
                            {record &&
                              ...
                              record.status !== 'absent' && (
                                <Button
                                  ...
                                  onClick={() =>
                                    markCheckout.mutate(student.id)
                                  }
                                >
                                  Out
                                </Button>
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base font-semibold flex items-center gap-2">
              <Sun className="w-4 h-4" /> Monthly Report —{' '}
          ...
          <Table>
            ...
            <TableBody>
              {students.map((student) => {
                const stat = monthlyStats[student.id] || {
                  present: 0,
                  absent: 0,
                  late: 0,
                  total: 0,
                ...
                return (
                  <TableRow key={student.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-xs ${pct >= 75 ? 'text-green-600' : pct >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {pct}%
                      </Badge>
                    </TableCell>
