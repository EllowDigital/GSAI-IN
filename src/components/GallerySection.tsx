...
export default function GallerySection() {
  ...
  return (
    <section
      id="gallery"
      className="py-12 xs:py-16 md:py-20 px-2 xs:px-3 md:px-4 bg-gradient-to-br from-yellow-50 via-white to-red-50 border-b border-yellow-100"
    >
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col items-center mb-7 gap-2">
          ...
          <p className="text-base md:text-lg font-medium text-gray-500 text-center max-w-xl">
            Dive into moments of training, triumph, and togetherness! Browse
            photos from recent sessions, events, and community fun.
          </p>
        </div>
        {loading ? (
          ...
          <div className="text-gray-400 text-center py-16 font-semibold">
            No images yet.
          </div>
        ) : (
          <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-4 gap-3 xs:gap-5 md:gap-7">
            {images.map((img, i) => (
              <img
                ...
                alt={
                  img.caption
                    ? `GSAI photo: ${img.caption}`
                    : `Academy event/training photo ${i + 1}`
                }
                loading="lazy"
