...
export default function DashboardHome() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [
        studentsRes, feesRes, blogsRes, newsRes, eventsRes, galleryRes, enrollRes, announcementsRes, attendanceRes,
      ] = await Promise.all([
  ...
  const quickActions = [
    { title: 'New Enrollment', icon: UserPlus, path: '/admin/dashboard/enrollments', desc: 'Review requests' },
    ...
    { title: 'Announcement', icon: MessageSquare, path: '/admin/dashboard/announcements', desc: 'Post notice' },
    { title: 'Create Event', icon: CalendarPlus, path: '/admin/dashboard/events', desc: 'Schedule event' },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {quickActions.map((action) => (
            ...
            >
              ...
              <div>
                <span className="text-xs font-semibold text-foreground leading-tight block">{action.title}</span>
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Last 6 months collection trend</p>
            </div>
            <Link
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Student distribution</p>
            </div>
            <Link
          ...
          <div className="space-y-4">
            {analytics.programs.map((program) => {
              ...
              const percentage = Math.round((program.count / analytics.totalStudents) * 100);
              return (
                <div key={program.name} className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-muted-foreground tabular-nums">{program.count}</span>
                      <span className="text-[10px] text-muted-foreground/60 tabular-nums w-8 text-right">{percentage}%</span>
                    </div>
                  </div>
            ...
            {analytics.programs.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-8">No programs yet</p>
            )}
          </div>
        </div>
      </div>
