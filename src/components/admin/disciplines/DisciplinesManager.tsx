...
import {
  GraduationCap, Plus, Pencil, Trash2, Loader2, Award, Layers, GripVertical,
} from 'lucide-react';
...
export default function DisciplinesManager() {
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase.from('disciplines').update({ is_active }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['disciplines-admin'] });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage all martial arts disciplines — add, rename, delete, or toggle active status
          </p>
        </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-auto">{beltDisciplines.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {beltDisciplines.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">No belt-based disciplines</p>
              ) : (
                beltDisciplines.map((d) => (
                  <DisciplineRow
                    ...
                    onToggleActive={(active) => toggleActiveMutation.mutate({ id: d.id, is_active: active })}
                  />
                ))
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-auto">{levelDisciplines.length}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {levelDisciplines.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">No level-based disciplines</p>
              ) : (
      ...
      <Dialog open={formOpen} onOpenChange={(o) => !o && closeForm()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? 'Edit Discipline' : 'Add Discipline'}</DialogTitle>
            <DialogDescription>
              {editing ? 'Update discipline details.' : 'Add a new martial arts discipline.'}
            </DialogDescription>
          </DialogHeader>
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-2">
              <Label>Name</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Taekwondo" />
            </div>
            <div className="space-y-2">
              <Label>Type</Label>
              <Select value={type} onValueChange={(v) => setType(v as 'belt' | 'level')}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{deleteTarget?.name}"? This won't remove associated belts or levels from the database, but the discipline entry will be gone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function DisciplineRow({
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-lg border border-border/50 transition-colors ${discipline.is_active ? 'bg-muted/20 hover:bg-muted/40' : 'bg-muted/5 opacity-60'}`}>
      <GripVertical className="h-4 w-4 text-muted-foreground/50 flex-shrink-0" />
      <Badge variant="outline" className="text-xs px-2 py-0.5 font-mono flex-shrink-0">
        #{discipline.display_order}
      </Badge>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-foreground truncate">{discipline.name}</p>
          {discipline.has_stripes && (
            <Badge variant="secondary" className="text-[10px]">Stripes</Badge>
          )}
        ...
        {discipline.description && (
          <p className="text-xs text-muted-foreground truncate">{discipline.description}</p>
        )}
      </div>
      <Switch
