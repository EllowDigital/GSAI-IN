...
export default function StudentProfileCard() {
  ...
  const { data: student, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('students')
        .select(
          'name, program, join_date, profile_image_url, parent_name, parent_contact'
        )
  ...
  const handleSaveProfile = async () => {
    ...
    if (
      editForm.parent_contact &&
      !/^[6-9]\d{9}$/.test(editForm.parent_contact)
    ) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 p-4 sm:p-5">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              ...
              <div>
                ...
                <Badge
                  variant="outline"
                  className={`${beltStyle} border capitalize`}
                >
                  {beltColor} Belt
                  {currentBelt?.stripe_count > 0 &&
                    ` • ${currentBelt.stripe_count} stripe${currentBelt.stripe_count > 1 ? 's' : ''}`}
                </Badge>
              </div>
            </div>
          </div>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Programs and join date can only be changed by admin.
            </p>
