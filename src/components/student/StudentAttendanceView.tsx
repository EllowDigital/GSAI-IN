...
export default function StudentAttendanceView() {
  ...
  const { data: attendance = [], isLoading } = useQuery({
    queryKey: ['student-attendance', profile?.studentId, format(currentMonth, 'yyyy-MM')],
    queryFn: async () => {
  ...
  const stats = useMemo(() => {
    let present = 0, absent = 0, late = 0, total = (attendance as any[]).length;
    (attendance as any[]).forEach((a: any) => {
      if (a.status === 'present' || a.status === 'half_day') present++;
      if (a.status === 'absent') absent++;
      if (a.status === 'late') { late++; present++; }
    });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
          <CalendarCheck className="w-4 h-4" /> Attendance — {format(currentMonth, 'MMMM yyyy')}
        </h3>
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setMonthOffset(m => m - 1)}>
            <ChevronLeft className="w-3.5 h-3.5" />
          </Button>
          <Button variant="ghost" size="sm" className="text-xs h-7" onClick={() => setMonthOffset(0)}>Current</Button>
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setMonthOffset(m => m + 1)}>
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <Card className={stats.pct >= 75 ? 'border-green-500/20' : stats.pct >= 50 ? 'border-yellow-500/20' : 'border-red-500/20'}>
          <CardContent className="p-2.5 text-center">
            <p className={`text-lg font-bold ${stats.pct >= 75 ? 'text-green-600' : stats.pct >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>{stats.pct}%</p>
      ...
      {(attendance as any[]).length === 0 ? (
        <p className="text-center text-sm text-muted-foreground py-6">No attendance records for this month.</p>
      ) : (
        <div className="space-y-2">
          {(attendance as any[]).map((record: any) => {
            const statusInfo = STATUS_DISPLAY[record.status] || STATUS_DISPLAY.present;
            return (
              <Card key={record.id} className="border border-border">
                <CardContent className="p-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{format(new Date(record.date), 'EEE, MMM d')}</p>
                    <div className="flex gap-3 text-xs text-muted-foreground mt-0.5">
                      {record.check_in_time && <span>In: {format(new Date(record.check_in_time), 'h:mm a')}</span>}
                      {record.check_out_time && <span>Out: {format(new Date(record.check_out_time), 'h:mm a')}</span>}
                    </div>
                  </div>
