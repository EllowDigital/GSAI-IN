...
import {
  Edit, Trash2, User, Award, Calendar, Phone, Users, KeyRound,
} from 'lucide-react';
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <Card key={i} className="animate-pulse bg-card">
            <CardContent className="p-4">
              <div className="flex gap-3"><div className="w-12 h-12 bg-muted rounded-full" /><div className="flex-1 space-y-2"><div className="h-4 bg-muted rounded w-3/4" /><div className="h-3 bg-muted rounded w-1/2" /></div></div>
            </CardContent>
          </Card>
        ))}
      </div>
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {studentsWithBelts.map(stu => {
          ...
          return (
            <Card key={stu.id} className="group rounded-xl sm:rounded-2xl bg-card border border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300 overflow-hidden">
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    {!(portalAccountIds as Set<string>).has(stu.id) ? (
                      <Button size="icon" variant="ghost"
                        ...
                        className="h-8 w-8 rounded-lg hover:bg-accent" title="Create Portal Account">
                        <KeyRound className="w-4 h-4" />
                      </Button>
                    ) : (
                      <Badge variant="outline" className="text-[10px] h-6 px-1.5 border-green-500/30 text-green-600">Portal ✓</Badge>
                    )}
                    <Button size="icon" variant="ghost" onClick={() => onEdit(stu)} className="h-8 w-8 rounded-lg hover:bg-primary/10 hover:text-primary"><Edit className="w-4 h-4" /></Button>
                    <Button size="icon" variant="ghost" onClick={() => onDelete(stu)} className="h-8 w-8 rounded-lg hover:bg-destructive/10 hover:text-destructive"><Trash2 className="w-4 h-4" /></Button>
                  </div>
                </div>
      ...
      <CreatePortalAccountDialog open={!!portalStudent} onOpenChange={val => { if (!val) setPortalStudent(null); }} student={portalStudent} />
    </>
  );
}
