...
export default function StudentAvatarUploader({ url, onUploaded }: StudentAvatarUploaderProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!_data || !_data.publicUrl) throw new Error("Unable to get public URL.");
  ...
  return (
    <div className="flex gap-4 items-center">
      <div className="h-16 w-16 rounded-full bg-gray-100 flex items-center justify-center border">
        {url ? (
          <img src={url} alt="Avatar" className="h-full w-full object-cover rounded-full" />
        ) : (
