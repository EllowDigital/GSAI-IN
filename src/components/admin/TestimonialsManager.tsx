...
export default function TestimonialsManager() {
  ...
  const [form, setForm] = useState({ program_slug: '', student_name: '', review: '', rating: 5 });
  ...
  const togglePublish = async (id: string, current: boolean) => {
    await supabase.from('program_testimonials' as any).update({ is_published: !current } as any).eq('id', id);
    fetchAll();
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Delete this testimonial?')) return;
    await supabase.from('program_testimonials' as any).delete().eq('id', id);
    fetchAll();
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl sm:text-2xl font-bold text-foreground">Program Testimonials</h2>
        <button
      ...
      {showForm && (
        <div className="bg-muted/50 rounded-xl p-4 sm:p-6 border border-border space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm((f) => ({ ...f, program_slug: e.target.value }))}
          ...
          <div className="flex gap-3">
            <button onClick={handleAdd} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-semibold text-sm">Save</button>
            <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-lg bg-muted text-foreground font-semibold text-sm border border-border">Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground text-sm">No testimonials yet. Add your first one!</p>
      ) : (
        <div className="grid gap-4">
          {testimonials.map((t) => {
            const prog = programs.find((p) => p.slug === t.program_slug);
            return (
              <div key={t.id} className="bg-card rounded-xl p-4 border border-border flex flex-col sm:flex-row sm:items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-semibold text-foreground text-sm">{t.student_name}</span>
                    <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">{prog?.title ?? t.program_slug}</span>
                    {!t.is_published && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-600 font-medium">Draft</span>
                    )}
                  </div>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => togglePublish(t.id, t.is_published)} title={t.is_published ? 'Unpublish' : 'Publish'} className="p-2 rounded-lg hover:bg-muted transition-colors">
                    {t.is_published ? <Eye className="w-4 h-4 text-green-500" /> : <EyeOff className="w-4 h-4 text-gray-400" />}
                  </button>
                  <button onClick={() => handleDelete(t.id)} className="p-2 rounded-lg hover:bg-destructive/10 transition-colors">
