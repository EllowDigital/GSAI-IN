...
import {
  Users, DollarSign, BookOpen, Newspaper, Calendar, Image,
...
export default function DashboardHome() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [studentsRes, feesRes, blogsRes, newsRes, eventsRes, galleryRes] = await Promise.all([
        supabase.from('students').select('id, program, join_date, created_at'),
        supabase.from('fees').select('id, status, paid_amount, monthly_fee, month, year'),
  ...
  const analytics = React.useMemo(() => {
    ...
    const totalRevenue = data.fees.filter(f => f.status === 'paid').reduce((sum, f) => sum + (f.paid_amount || 0), 0);
    ...
    const programData = Object.entries(programStats).map(([name, value]) => ({ name, value }));
    ...
    const monthlyRevenue = (() => {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const revenue = data.fees.filter(f => {
          return f.year === date.getFullYear() && f.month === date.getMonth() + 1 && f.status === 'paid';
  ...
  const statCards = [
    { title: 'Total Students', value: analytics.totals.totalStudents, icon: Users, trend: '+12%', positive: true },
    { title: 'Total Revenue', value: `₹${analytics.totals.totalRevenue.toLocaleString()}`, icon: DollarSign, trend: '+8%', positive: true },
    { title: 'Paid Fees', value: analytics.totals.paidFees, icon: TrendingUp, trend: '+15%', positive: true },
    { title: 'Blog Posts', value: analytics.totals.totalBlogs, icon: BookOpen, trend: '+5%', positive: true },
    { title: 'News Articles', value: analytics.totals.totalNews, icon: Newspaper, trend: '+3%', positive: true },
    { title: 'Events', value: analytics.totals.totalEvents, icon: Calendar, trend: '+20%', positive: true },
    { title: 'Gallery Images', value: analytics.totals.totalGallery, icon: Image, trend: '+10%', positive: true },
    { title: 'Unpaid Fees', value: analytics.totals.unpaidFees, icon: TrendingDown, trend: '-5%', positive: false }
  ];
    ...
    <div className="w-full min-h-full p-2 sm:p-4 lg:p-6 xl:p-8 space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
        {[
          ...
          { title: 'Events', icon: Calendar, path: '/admin/dashboard/events' },
          ...
          <Card key={index} className="group cursor-pointer hover:shadow-lg transition-all duration-300 hover:scale-105">
            <CardContent className="p-3 sm:p-4 text-center space-y-2">
              ...
              <div>
                <h3 className="font-semibold text-xs sm:text-sm text-foreground">{action.title}</h3>
              </div>
            </CardContent>
          </Card>
        ))}
