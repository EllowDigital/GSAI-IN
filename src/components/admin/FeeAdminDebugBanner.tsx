...
export function FeeAdminDebugBanner({
  adminDebug,
}: {
  adminDebug?: {
  ...
  const cannotSubmitReason = !adminDebug.canSubmitFeeEdits
  ...
  return (
    <div className="mb-2 px-2 py-1 rounded bg-gray-50 border text-xs text-gray-700">
      <div>
        <b>Debug Info:</b> <span>Email:</span>{' '}
        <span className="font-mono">{adminDebug.adminEmail || 'N/A'}</span>
        {' | '}
        <span>In admin_users:</span>{' '}
        <span className="font-bold">
          {adminDebug.isAdminInTable ? '✅' : '❌'}
        </span>
        {' | '}
        <span>Can submit fees:</span>{' '}
        <span className="font-bold">
