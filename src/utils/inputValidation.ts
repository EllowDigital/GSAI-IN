 ...
 */
export function validateAadharNumber(aadhar: string): { isValid: boolean; sanitized: string } {
 ...
 */
export function validateAmount(amount: string | number): { isValid: boolean; value: number } {
  const numericValue = typeof amount === 'string' ? parseFloat(amount) : amount;
  
  return {
    isValid: !isNaN(numericValue) && numericValue >= 0 && numericValue <= 1000000, // Max 10 lakh
    value: numericValue
  };
}
 ...
 */
export function validateDate(dateString: string): { isValid: boolean; date: Date | null } {
  try {
