  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        <div className="absolute top-20 left-10 w-64 h-64 md:w-72 md:h-72 bg-yellow-200/40 rounded-full blur-3xl" />
        <div className="absolute bottom-20 right-10 w-72 h-72 md:w-80 md:h-80 bg-orange-200/30 rounded-full blur-3xl" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 md:w-64 md:h-64 bg-amber-200/20 rounded-full blur-3xl" />
      </div>

      {/* Floating Icons */}
      <Sparkles className="absolute top-16 right-8 w-6 h-6 sm:w-8 sm:h-8 text-yellow-400 opacity-30 animate-float" />
      <Calendar
        className="absolute bottom-20 left-8 w-8 h-8 sm:w-10 sm:h-10 text-amber-500 opacity-25 animate-float"
      ...
      <div className="section-stack relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!error && !isLoading && eventList.length === 0 && (
          ...
          >
            ...
            <div className="inline-flex items-center gap-2 text-yellow-600 font-semibold text-sm sm:text-base">
            ...
            <AnimatePresence>
              {eventList.map((event, idx) => {
                ...
                return (
                  <motion.div
                    key={event.id}
                    className="group relative bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden hover:shadow-2xl hover:border-yellow-200 transition-all transform hover:-translate-y-2 flex flex-col h-full"
                  ...
                  >
                    ...
                    <div className="relative h-48 sm:h-56 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-white/95 backdrop-blur-sm rounded-lg px-2.5 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm font-semibold shadow-md text-gray-800">
                        <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 inline-block mr-1.5 text-yellow-600" />
                        {date.single
                          ? date.start
                          : `${date.start} - ${date.end}`}
                      </div>
                      {event.tag && (
                        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-gradient-to-r from-yellow-500 to-amber-600 text-white px-2.5 py-1 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold shadow-lg uppercase tracking-wide">
                          {event.tag}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-5 sm:p-6 flex flex-col flex-grow">
                      <div className="flex items-center text-xs sm:text-sm text-gray-500 mb-2 font-medium">
                        <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 text-yellow-500" />
                        Upcoming Event
                      </div>
                      <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-yellow-700 transition-colors">
                        {event.title}
                      </h3>
                      <p className="text-gray-600 text-sm leading-relaxed line-clamp-3 mb-4 flex-grow">
                        {event.description}
                      </p>
                      <div className="flex items-center justify-between text-xs sm:text-sm border-t border-gray-100 pt-4 text-gray-500 group-hover:text-yellow-600 transition-colors mt-auto">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 cursor-pointer font-bold text-yellow-600 hover:text-orange-600 transition-colors"
                        >
                          Read More
        ...
        {eventList.length > 0 && (
          ...
          >
            <div className="inline-flex flex-col sm:flex-row items-center gap-4 bg-white/50 backdrop-blur-sm p-6 rounded-2xl border border-white/60 shadow-sm">
              <button
