...
export default function DashboardHome() {
    ...
    queryFn: async () => {
      const [
        ...
        supabase
          ...
          .order('created_at', { ascending: false }),
        supabase.from('blogs').select('id, created_at, title').order('created_at', { ascending: false }),
        supabase.from('news').select('id, created_at, status, title').order('created_at', { ascending: false }),
        supabase.from('events').select('id, date, title').order('date', { ascending: false }),
        supabase.from('gallery_images').select('id, created_at, tag').order('created_at', { ascending: false }),
        ...
        supabase
          ...
          .then(res => {
            ...
            const programCounts = programs.reduce((acc, student) => {
              acc[student.program] = (acc[student.program] || 0) + 1;
              return acc;
            }, {} as Record<string, number>);
            return Object.entries(programCounts).map(([name, value]) => ({ name, value, percentage: 0 }));
          }),
  ...
  const analytics = React.useMemo(() => {
    ...
    const programAnalytics = (() => {
      const programStats = data.students.reduce((acc, student) => {
    ...
    // Content Performance 
    const contentStats = (() => {
      const recentBlogs = data.blogs.filter(b => 
        new Date(b.created_at) >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
      ).length;
      
      const recentNews = data.news.filter(n => 
        new Date(n.created_at) >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
      ).length;

      const recentGallery = data.gallery.filter(g => 
        new Date(g.created_at) >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
      ).length;
  ...
  const statCards = [
    {
      ...
      icon: Users,
      trend: analytics.growthAnalytics.length > 1 ? 
        `${(analytics.growthAnalytics[analytics.growthAnalytics.length - 1]?.growth || 0).toFixed(1)}%` : '+0%',
      positive: (analytics.growthAnalytics[analytics.growthAnalytics.length - 1]?.growth || 0) >= 0,
    ...
    },
    {
      ...
      icon: TrendingUp,
      trend: analytics.totals.unpaidFees === 0 ? '100%' : 
    ...
    },
    {
      ...
      icon: Activity,
      trend: analytics.contentStats.totalActivity > 5 ? 'High' : analytics.contentStats.totalActivity > 2 ? 'Medium' : 'Low',
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={analytics.growthAnalytics}>
                  ...
                  <Tooltip 
                    formatter={(value, name) => [
                      name === 'enrollments' ? `${value} new students` : 
                      name === 'total' ? `${value} total students` : 
                      `${Number(value).toFixed(1)}% growth`,
                      name === 'enrollments' ? 'New Enrollments' :
                      name === 'total' ? 'Total Students' : 'Growth Rate'
                    ]}
                  />
                  <Bar yAxisId="left" dataKey="enrollments" fill="hsl(var(--primary))" opacity={0.7} />
                  <Line 
                    yAxisId="right" 
                    type="monotone" 
                    dataKey="total" 
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                { title: 'Students', icon: Users, path: '/admin/dashboard/students', color: 'blue' },
                { title: 'Fees', icon: DollarSign, path: '/admin/dashboard/fees', color: 'green' },
                { title: 'Blogs', icon: BookOpen, path: '/admin/dashboard/blogs', color: 'purple' },
                { title: 'News', icon: Newspaper, path: '/admin/dashboard/news', color: 'red' },
                { title: 'Events', icon: Calendar, path: '/admin/dashboard/events', color: 'orange' },
                { title: 'Gallery', icon: Image, path: '/admin/dashboard/gallery', color: 'pink' },
              ].map((action, index) => (
            ...
            <div className="space-y-3">
              <div className="p-3 bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-950/20 dark:to-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium text-emerald-800 dark:text-emerald-200">Collection Efficiency</span>
                </div>
                <p className="text-xs text-emerald-700 dark:text-emerald-300">
                  {analytics.collectionEfficiency.efficiency.toFixed(1)}% of expected revenue collected
                </p>
              </div>
              ...
              <div className="p-3 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950/20 dark:to-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium text-blue-800 dark:text-blue-200">Content Activity</span>
                </div>
                <p className="text-xs text-blue-700 dark:text-blue-300">
                  {analytics.contentStats.totalActivity} content pieces added this month
                </p>
              </div>
              ...
              <div className="p-3 bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-950/20 dark:to-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium text-purple-800 dark:text-purple-200">Top Program</span>
                </div>
