...
export function FeeForm({
  ...
  const effectiveStatus = statusOverrideWatched === 'auto' ? autoStatus : statusOverrideWatched;
  ...
  async function onSubmit(values: {
    ...
    if (!student || typeof student.id !== 'string') {
      toast({ title: 'Student data missing', description: 'Cannot save fee record without a valid student.', variant: 'error' });
      return;
    }
    if (paid_amount > monthly_fee + carryForward) {
      toast({ title: 'Invalid Paid Amount', description: 'Paid amount cannot exceed monthly fee plus carry-forward balance.', variant: 'error' });
      return;
    }
    if (monthly_fee < 0 || paid_amount < 0) {
      toast({ title: 'Invalid Amount', description: 'Monthly fee and paid amount must be non-negative.', variant: 'error' });
      return;
    }
    ...
    const { data: result, error } = await safeAsync(async () => {
      ...
      const basePayload = {
        ...
        updated_at: now,
        status: values.status_override === 'auto' ? autoStatus : values.status_override,
      };
      const payload = fee && fee.id ? { ...basePayload } : { ...basePayload, created_at: now };

      if (fee && fee.id) {
        const { data, error } = await supabase.from('fees').update(payload).eq('id', fee.id).select().maybeSingle();
        if (error) throw error;
        return data;
      } else {
        const { data, error } = await supabase.from('fees').upsert([payload], { onConflict: 'student_id,month,year' }).select().maybeSingle();
        if (error) throw error;
        return data;
      }
    }, 'Fee Form Submission');
    ...
    setLoading(false);
    if (error) {
      toast({ title: 'Failed to save fee', description: formatErrorForDisplay(error), variant: 'error' });
    } else {
      toast({ title: 'Fee saved successfully', description: `Fee record for ${student.name} has been ${fee?.id ? 'updated' : 'created'}.` });
      onClose();
    }
  }
    ...
    <form className="space-y-5" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border/50">
        {student?.profile_image_url ? (
          <img src={student.profile_image_url} alt="" className="w-10 h-10 rounded-full object-cover ring-2 ring-primary/20" />
        ) : (
        ...
        <div className="text-right">
          <span className="text-xs font-medium text-muted-foreground">{MONTH_NAMES[month - 1]} {year}</span>
          {discountPercent > 0 && (
      ...
      {carryForward > 0 && (
        <div className="flex items-center gap-2 p-2.5 rounded-lg bg-yellow-50 border border-yellow-200 text-xs">
          <span className="text-yellow-800">
            ⚠️ Carried forward from previous month: <strong>₹{carryForward.toLocaleString('en-IN')}</strong>
          </span>
        </div>
      )}
      ...
      {/* Status Override */}
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-foreground">Status Override</label>
        <Select
      ...
      <div className="flex justify-end gap-2 pt-2 border-t border-border/50">
        ...
        <Button type="submit" disabled={loading} className="min-w-[80px]">
          {loading ? <Loader2 className="animate-spin w-4 h-4" /> : fee?.id ? 'Update' : 'Save'}
        </Button>
      </div>
    </form>
  );
