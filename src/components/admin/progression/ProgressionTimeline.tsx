...
interface PromotionHistoryItem {
  ...
  students?: { id: string; name: string; profile_image_url: string | null } | null;
}
...
export default function ProgressionTimeline({ history, isLoading }: ProgressionTimelineProps) {
  if (isLoading) {
    return (
  ...
  if (history.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        ...
        <p className="text-xs mt-1">Promotions will appear here as students progress</p>
      </div>
    );
  }
            ...
            return (
              <div key={item.id} className="relative pl-14">
                ...
                <div className="absolute left-4 top-4 z-10">
                  <div className={`h-5 w-5 rounded-full border-2 border-background shadow-sm ${toStyle.bg} ${toStyle.border}`}>
                ...
                {/* Card */}
                <div className={`rounded-xl border bg-card p-4 transition-all hover:shadow-md ${index === 0 ? 'ring-2 ring-primary/20' : ''}`}>
                  {/* Header */}
                  <div className="flex items-start gap-3 mb-3">
                    <Avatar className="h-10 w-10 flex-shrink-0">
                      {item.students?.profile_image_url ? (
                        <AvatarImage src={item.students.profile_image_url} alt={item.students?.name} />
                      ) : (
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${fromStyle.bg} ${fromStyle.text} ${fromStyle.border}`}>
                      <Award className="h-3 w-3" />
                      {item.from_belt?.color ?? 'Start'}
                    </span>
                    <ArrowRight className="h-4 w-4 text-primary" />
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${toStyle.bg} ${toStyle.text} ${toStyle.border}`}>
                      <Award className="h-3 w-3" />
