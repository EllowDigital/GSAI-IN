...
import { Edit, Trash2, User, Award, Calendar, Phone, Users, KeyRound } from 'lucide-react';
...
export default function StudentsCards({ students, onEdit, onDelete, loading }: Props) {
  ...
  const { data: portalAccountIds = new Set<string>() } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = (await supabase.from('student_portal_accounts').select('student_id')) as any;
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3">
        {studentsWithBelts.map((stu) => {
          ...
          return (
            <Card key={stu.id} className="group rounded-xl bg-card border border-border/50 hover:border-primary/20 hover:shadow-md transition-all duration-200 overflow-hidden">
              <CardContent className="p-3 sm:p-4 flex flex-col gap-2.5">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm sm:text-base text-foreground truncate">{stu.name}</h3>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {allPrograms.map((prog, i) => (
                        <Badge key={prog} variant={i === 0 ? 'default' : 'secondary'} className="text-[9px] px-1.5 py-0">{prog}</Badge>
                      ))}
                    </div>
                    <div className="mt-1.5">
                      <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium border ${getBeltColorClass(stu.belt_color ?? 'white')}`}>
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    {!(portalAccountIds as Set<string>).has(stu.id) ? (
                      <Button size="icon" variant="ghost" onClick={() => setPortalStudent({ id: stu.id, name: stu.name, program: stu.program })} className="h-7 w-7 rounded-lg" title="Create Portal Account">
                        <KeyRound className="w-3.5 h-3.5" />
                      </Button>
                    ) : (
                      <Badge variant="outline" className="text-[9px] h-5 px-1 border-emerald-500/30 text-emerald-600">Portal ✓</Badge>
                    )}
                    <Button size="icon" variant="ghost" onClick={() => onEdit(stu)} className="h-7 w-7 rounded-lg hover:bg-primary/10 hover:text-primary">
                      <Edit className="w-3.5 h-3.5" />
                    </Button>
                    <Button size="icon" variant="ghost" onClick={() => onDelete(stu)} className="h-7 w-7 rounded-lg hover:bg-destructive/10 hover:text-destructive">
